( function ( $, L, swpmb, i18n ) {
	...
	OsmField.prototype = {
		...
		initDomElements: function () {
			this.$canvas = this.$container.find( '.swpmb-osm-canvas' );
			this.canvas = this.$canvas[ 0 ];
			this.$coordinate = this.$container.find( '.swpmb-osm' );
	...
	function init( e ) {
		$( e.target ).find( '.swpmb-osm-field' ).each( createController );
	}

	function restart() {
		$( '.swpmb-osm-field' ).each( createController );
	}

	swpmb.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.swpmb-input', restart );
} )( jQuery, L, swpmb, SWPMB_Osm );