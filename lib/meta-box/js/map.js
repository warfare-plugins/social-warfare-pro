( function ( $, document, window, google, swpmb, i18n ) {
	'use strict';
	...
	MapField.prototype = {
		...
		initDomElements: function () {
			this.$canvas = this.$container.find( '.swpmb-map-canvas' );
			this.canvas = this.$canvas[ 0 ];
			this.$coordinate = this.$container.find( '.swpmb-map' );
		...
		addListeners: function () {
			...
			$( window ).on( 'swpmb_map_refresh', refresh );

			// Refresh on meta box hide and show
			swpmb.$document.on( 'postbox-toggled', refresh );
		...
		findAddressField: function ( fieldName ) {
			...
			$address = this.$container.closest( '.swpmb-group-clone' ).find( 'input[name*="[' + fieldName + ']"]' );
			if ( $address.length ) {
				return $address;
			}

			// If map and address is inside a non-cloneable group.
			$address = this.$container.closest( '.swpmb-group-wrapper' ).find( 'input[name*="[' + fieldName + ']"]' );
			if ( $address.length ) {
				return $address;
			}
	...
	function init( e ) {
		$( e.target ).find( '.swpmb-map-field' ).each( createController );
	}

	function restart() {
		$( '.swpmb-map-field' ).each( createController );
	}

	swpmb.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.swpmb-input', restart );
} )( jQuery, document, window, google, swpmb, SWPMB_Map );
