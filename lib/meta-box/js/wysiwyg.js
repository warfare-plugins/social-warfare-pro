( function( $, wp, window, swpmb ) {
	...
	function transform() {
		...
		var originalId = getOriginalId( this ),
			...
			customSettings = $this.closest( '.swpmb-input' ).find( '.swpmb-wysiwyg-id' ).data( 'options' );
	...
	function getOriginalId( el ) {
		return el.closest( '.swpmb-input' ).querySelector( '.swpmb-wysiwyg-id' ).dataset.id;
	}
	...
	function init( e ) {
		$( e.target ).find( '.swpmb-wysiwyg' ).each( transform );
	}
	...
	function addRequiredAttribute() {
		if ( this.classList.contains( 'swpmb-wysiwyg-required' ) ) {
	...
	swpmb.$document
		...
		.on( 'clone', '.swpmb-wysiwyg', function() {
			/*
