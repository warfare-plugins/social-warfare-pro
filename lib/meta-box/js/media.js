( function ( $, wp, _, swpmb, i18n ) {
	'use strict';

	var views = swpmb.views = swpmb.views || {},
		models = swpmb.models = swpmb.models || {},
		media = wp.media,
	...
	MediaCollection = Backbone.Collection.extend( {
		...
		remove: function ( models, options ) {
			...
			if( ! $( event.target ).closest( '.swpmb-field, [data-class="swpmb-field"]' ).length ) {
				return;
			}
	...
	MediaField = views.MediaField = Backbone.View.extend( {
		...
		initialize: function ( options ) {
			...
			collection.on( 'all', _.debounce( function() {
				...
				that.$input.val( ids ).trigger( 'change', [that.$( '.swpmb-media-input' )] );
			}, 500 ) );
		},
	...
	MediaButton = views.MediaButton = Backbone.View.extend( {
		tagName: 'div',
		className: 'swpmb-media-add',
		template: wp.template( 'swpmb-media-button' ),
		events: {
	...
	MediaItem = views.MediaItem = Backbone.View.extend( {
		...
		events: {
			'click .swpmb-image-overlay': function ( e ) {
				e.preventDefault();
			...
			'click .swpmb-edit-media': function ( e ) {
				e.preventDefault();
	...
	MediaSelect = views.MediaSelect = MediaFrame.Select.extend( {
		...
		createStates: function () {
			...
			this.$el.attr( 'data-class', 'swpmb-field' );
	...
	function init( e ) {
		$( e.target ).find( '.swpmb-file_advanced' ).each( initMediaField );
	}

	swpmb.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.swpmb-file_advanced', initMediaField );
} )( jQuery, wp, _, swpmb, i18nRwmbMedia );
