( function ( $, _, swpmb, i18n ) {
	...
	function transform() {
		...
		let $timestamp = $this.siblings( '.swpmb-datetime-timestamp' ),
			current = $this.val(),
	...
	function init( e ) {
		 ...
		 */
		setTimeout( () => {
			$( e.target ).find( '.swpmb-datetime' ).each( transform );
		}, 0 );
	}
	...
	swpmb.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.swpmb-datetime', transform );
} )( jQuery, _, swpmb, SWPMB_Datetime );
