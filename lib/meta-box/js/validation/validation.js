( function ( $, swpmb, i18n ) {
	...
	$.validator.staticRules = function ( element ) {
		 ...
		 */
		if ( element.type === 'file' && ( $( element ).closest( '.swpmb-clone' ).length > 0 || $( element ).closest( '.swpmb-group-wrapper' ).length > 0 ) ) {
			const $input = $( element ).closest( '.swpmb-input' );
	...
	class Validation {
		constructor( formSelector ) {
			...
			this.validationElements = this.$form.find( '.swpmb-validation' );
			this.showAsterisks();
			this.getSettings();
		}
		...
		showAsterisks() {
			this.validationElements.each( function () {
				...
				$.each( data.rules, function ( k, v ) {
					...
					if ( $el.length ) {
						$el.closest( '.swpmb-input' ).siblings( '.swpmb-label' ).find( 'label' ).append( '<span class="swpmb-required">*</span>' );
					}
				} );
			} );
		}
		...
		getSettings() {
			this.settings = {
				ignore: ':not(.swpmb-media,.swpmb-image_select,.swpmb-wysiwyg,.swpmb-color,.swpmb-map,.swpmb-osm,.swpmb-switch,[class|="swpmb"]), .swpmb-clone-template *',
				errorPlacement: function ( error, element ) {
					error.appendTo( element.closest( '.swpmb-input' ) );
				},
				errorClass: 'swpmb-error',
				errorElement: 'p',
		...
		invalidHandler() {
			...
			for ( var i = 0; i < this.$form.data( 'validator' ).errorList.length; i++ ) {
				$( '#' + this.$form.data( 'validator' ).errorList[ i ].element.id ).closest( '.swpmb-group-collapsed' ).removeClass( 'swpmb-group-collapsed' );
			}
		...
		showMessage() {
			...
			$( '#swpmb-validation-message' ).remove();
			this.$form.before( '<div id="swpmb-validation-message" class="notice notice-error is-dismissible"><p>' + i18n.message + '</p></div>' );
		}
	};
	...
	function init() {
		...
		var $forms = $( '#post, #edittag, #your-profile, .swpmb-form' );
		$forms.each( function () {
