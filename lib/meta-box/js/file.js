( function ( $, rwmb ) {
	...
	file.addHandler = function ( event ) {
		...
		file.updateVisibility.call( $fieldInput.find( '.swpmb-uploaded' ) );
	...
	file.deleteHandler = function ( event ) {
		...
		var $this = $( this ),
			...
			$uploaded = $this.closest( '.swpmb-uploaded' ),
	...
	file.resetClone = function() {
		var $this = $( this ),
			...
			$list = $clone.find( '.swpmb-uploaded' ),
			$key = $clone.find( '.swpmb-file-index' ),
			inputName = '_file_' + rwmb.uniqid();

		$list.empty();
		$clone.find( '.swpmb-file-input' ).attr( 'name', inputName + '[]' ).not( ':first' ).remove();
	...
	file.setRequired = function() {
		var $this = $( this ),
			$uploaded = $this.find( '.swpmb-uploaded' ),
	...
	function init( e ) {
		var $el = $( e.target ),
			$uploaded = $el.find( '.swpmb-uploaded' );

		$uploaded.each( file.sort );
		$uploaded.each( file.updateVisibility );

		$el.find( '.swpmb-file-wrapper' ).each( file.setRequired );
	}
