( function ( $, swpmb ) {
	...
	function transform() {
		...
		if ( options.ajax_data ) {
			...
			options.ajax.transport = function ( params, success, failure ) {
				...
				var actions = {
					'post': 'swpmb_get_posts',
					'taxonomy': 'swpmb_get_terms',
					'taxonomy_advanced': 'swpmb_get_terms',
					'user': 'swpmb_get_users'
				};
	...
	function init( e ) {
		$( e.target ).find( '.swpmb-select_advanced, .swpmb-icon' ).each( transform );
	}
	...
	swpmb.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.swpmb-select_advanced, .swpmb-icon', transform )
