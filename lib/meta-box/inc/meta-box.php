...
class RW_Meta_Box {
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'swpmb', SWPMB_CSS_URL . 'style.css', [], SWPMB_VER );
		wp_style_add_data( 'swpmb', 'path', SWPMB_CSS_DIR . 'style.css' );
		if ( is_rtl() ) {
			wp_enqueue_style( 'swpmb-rtl', SWPMB_CSS_URL . 'style-rtl.css', [], SWPMB_VER );
			wp_style_add_data( 'swpmb-rtl', 'path', SWPMB_CSS_DIR . 'style-rtl.css' );
		}

		wp_enqueue_script( 'swpmb', SWPMB_JS_URL . 'script.js', [ 'jquery' ], SWPMB_VER, true );

		// Load clone script conditionally.
		foreach ( $this->fields as $field ) {
			if ( $field['clone'] ) {
				wp_enqueue_script( 'swpmb-clone', SWPMB_JS_URL . 'clone.js', [ 'jquery-ui-sortable' ], SWPMB_VER, true );
				break;
			}
		}
		...
		// Auto save.
		if ( $this->autosave ) {
			wp_enqueue_script( 'swpmb-autosave', SWPMB_JS_URL . 'autosave.js', [ 'jquery' ], SWPMB_VER, true );
		}
	...
	public function validate() : bool {
		$nonce = swpmb_request()->filter_post( "nonce_{$this->id}" );
