 ...
 */
class SWPMB_Walker_Input_List extends SWPMB_Walker_Base {
	/**
	...
	public function start_el( &$output, $object, $depth = 0, $args = array(), $current_object_id = 0 ) {
		$attributes = SWPMB_Field::call( 'get_attributes', $this->field, $object->value );

		$output .= sprintf(
			'<li><label><input %s %s>%s</label>',
			SWPMB_Field::render_attributes( $attributes ),
			checked( in_array( $object->value, $this->meta ), true, false ),
			SWPMB_Field::filter( 'choice_label', $object->label, $this->field, $object )
		);
	}
