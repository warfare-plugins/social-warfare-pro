...
abstract class SWPMB_Field {
	...
	public static function show( $field, $saved, $post_id = 0 ) {
		...
		if ( $field['clone'] ) {
			$field_html = SWPMB_Clone::html( $meta, $field );
		} else {
	...
	public static function begin_html( $meta, $field ) {
		...
		if ( $field['name'] ) {
			$field_label = sprintf(
				...
				esc_attr( $field['id'] ),
				$field['name'],
				$field['required'] || ! empty( $field['attributes']['required'] ) ? '<span class="swpmb-required">*</span>' : '',
	...
	public static function end_html( $meta, $field ) {
		return SWPMB_Clone::add_clone_button( $field ) . self::call( 'input_description', $field ) . '</div>';
	}
	...
	public static function raw_meta( $object_id, $field, $args = array() ) {
		...
		if ( isset( $field['storage'] ) ) {
			...
			$storage = swpmb_get_storage( $args['object_type'] );
		} else {
			$storage = swpmb_get_storage( 'post' );
		}
	...
	public static function meta( $post_id, $saved, $field ) {
		...
		if ( $field['clone'] ) {
			$meta = SWPMB_Helpers_Array::ensure( $meta );
			...
			if ( $field['multiple'] ) {
				...
				$meta = is_array( $first ) ? array_map( 'SWPMB_Helpers_Array::ensure', $meta ) : array( $meta );
			}
	...
	public static function process_value( $value, $object_id, $field ) {
		...
		if ( $field['clone'] ) {
			$value = SWPMB_Clone::value( $value, $old_value, $object_id, $field );
		} else {
	...
	public static function save( $new, $old, $post_id, $field ) {
		...
		if ( ! SWPMB_Helpers_Value::is_valid_for_field( $new ) ) {
	...
	public static function get_attributes( $field, $value = null ) {
		...
		$attributes['class'] = trim( implode( ' ', array_merge( array( "swpmb-{$field['type']}" ), (array) $attributes['class'] ) ) );
	...
	public static function render_attributes( $attributes ) {
		...
		$attributes = array_filter( $attributes, 'SWPMB_Helpers_Value::is_valid_for_attribute' );
	...
	public static function call() {
		...
		return call_user_func_array( array( SWPMB_Helpers_Field::get_class( $field ), $method ), $args );
	}
	...
	public static function filter() {
		...
		if ( $field['id'] ) {
			...
			$filters[] = 'swpmb_' . $field_id . '_' . $name;
		}
