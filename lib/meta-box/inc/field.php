...
abstract class SWPMB_Field {
	...
	protected static function begin_html( array $field ): string {
		...
		$required = $field['required'] || ! empty( $field['attributes']['required'] );
		$required = $required ? '<span class="swpmb-required">*</span>' : '';

		$label = $field['name'] ? sprintf(
			// Translators: %1$s - field ID, %2$s - field label, %3$s - required asterisk, %4$s - label description.
			'<div class="swpmb-label" id="%1$s-label"><label for="%1$s">%2$s%3$s</label>%4$s</div>',
			esc_attr( $id ),
			$field['name'],
			$required,
	...
	protected static function end_html( array $field ): string {
		return SWPMB_Clone::add_clone_button( $field ) . static::input_description( $field ) . '</div>';
	}
	...
	public static function get_attributes( $field, $value = null ) {
		...
		$attributes['class'] = trim( implode( ' ', array_merge( [ "swpmb-{$field['type']}" ], (array) $attributes['class'] ) ) );
	...
	public static function call() {
		...
		$class = SWPMB_Helpers_Field::get_class( $field );
