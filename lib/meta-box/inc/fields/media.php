 ...
 */
class SWPMB_Media_Field extends SWPMB_File_Field {
	public static function admin_enqueue_scripts() {
		...
		wp_enqueue_style( 'swpmb-media', SWPMB_CSS_URL . 'media.css', [], SWPMB_VER );
		wp_style_add_data( 'swpmb-media', 'path', SWPMB_CSS_DIR . 'media.css' );
		wp_enqueue_script( 'swpmb-media', SWPMB_JS_URL . 'media.js', [ 'jquery-ui-sortable', 'underscore', 'backbone', 'media-grid' ], SWPMB_VER, true );

		SWPMB_Helpers_Field::localize_script_once( 'swpmb-media', 'i18nRwmbMedia', [
			'add'                => apply_filters( 'swpmb_media_add_string', _x( '+ Add Media', 'media', 'meta-box' ) ),
			'single'             => apply_filters( 'swpmb_media_single_files_string', _x( ' file', 'media', 'meta-box' ) ),
			'multiple'           => apply_filters( 'swpmb_media_multiple_files_string', _x( ' files', 'media', 'meta-box' ) ),
			'remove'             => apply_filters( 'swpmb_media_remove_string', _x( 'Remove', 'media', 'meta-box' ) ),
			'edit'               => apply_filters( 'swpmb_media_edit_string', _x( 'Edit', 'media', 'meta-box' ) ),
			'view'               => apply_filters( 'swpmb_media_view_string', _x( 'View', 'media', 'meta-box' ) ),
			'noTitle'            => _x( 'No Title', 'media', 'meta-box' ),
			'loadingUrl'         => admin_url( 'images/spinner.gif' ),
			'extensions'         => static::get_mime_extensions(),
			'select'             => apply_filters( 'swpmb_media_select_string', _x( 'Select Files', 'media', 'meta-box' ) ),
			'or'                 => apply_filters( 'swpmb_media_or_string', _x( 'or', 'media', 'meta-box' ) ),
			'uploadInstructions' => apply_filters( 'swpmb_media_upload_instructions_string', _x( 'Drop files here to upload', 'media', 'meta-box' ) ),
		] );
	}
