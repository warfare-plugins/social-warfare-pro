...
abstract class SWPMB_Object_Choice_Field extends SWPMB_Choice_Field {
	...
	public static function admin_enqueue_scripts() {
		...
		wp_enqueue_style( 'swpmb-modal', SWPMB_CSS_URL . 'modal.css', [], SWPMB_VER );
		wp_style_add_data( 'swpmb-modal', 'path', SWPMB_CSS_DIR . 'modal.css' );
		wp_enqueue_script( 'swpmb-modal', SWPMB_JS_URL . 'modal.js', [ 'jquery' ], SWPMB_VER, true );

		$type = $field['type'] === 'taxonomy_advanced' ? 'taxonomy' : $field['type'];
		wp_enqueue_script( "swpmb-$type", SWPMB_JS_URL . "$type.js", [ 'jquery', 'swpmb-modal' ], SWPMB_VER, true );
	}
	...
	protected static function get_type_class( array $field ) : string {
		return SWPMB_Helpers_Field::get_class( [ 'type' => $field['field_type'] ] );
	}
}
