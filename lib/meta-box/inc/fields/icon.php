 ...
 * The icon field.
 */
class SWPMB_Icon_Field extends SWPMB_Select_Advanced_Field {
	const CACHE_GROUP = 'meta-box-icon-field';

	public static function admin_enqueue_scripts() {
		parent::admin_enqueue_scripts();

		wp_enqueue_style( 'swpmb-icon', SWPMB_CSS_URL . 'icon.css', [], SWPMB_VER );
		wp_style_add_data( 'swpmb-icon', 'path', SWPMB_CSS_DIR . 'icon.css' );
		wp_enqueue_script( 'swpmb-icon', SWPMB_JS_URL . 'icon.js', [ 'swpmb-select2', 'swpmb-select', 'underscore' ], SWPMB_VER, true );
	...
	private static function enqueue_icon_font_style( array $field ): void {
		if ( is_string( $field['icon_css'] ) ) {
			...
			wp_enqueue_style( $handle, $field['icon_css'], [], SWPMB_VER );
	...
	public static function normalize( $field ) {
		...
		if ( $field['icon_set'] === 'font-awesome-pro' ) {
			...
			$field['icon_file'] = SWPMB_DIR . 'css/fontawesome/icons.json';
		}
